...
export function MyCertificatePage() {
  const { certificates, isLoading, error, reload } = useMyCertificate()
    ...
    <section className="space-y-5">
      ...
      <div className="grid gap-4">
        {certificates.map((certificate) => (
          <article key={certificate.certificateCode} className="space-y-4 rounded-lg border p-5">
              ...
              <div className="space-y-2">
                ...
                <div className="flex flex-wrap gap-2">
                  {certificate.skillsEarned.map((skill) => (
                    <Badge key={`${certificate.certificateCode}-${skill}`} variant="outline">
                      {skill}
                    </Badge>
                  ))}
                </div>
